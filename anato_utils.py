...
def GetFilteredMeshPaths(
    parent_folder, filter_strings, file_filter_strings, ext=".mat"
):
                ...
                for dir_name in dirs:
                    if dir_name == "mesh":
                        ...
                        for filename in mesh_folder.iterdir():
                            if filename.suffix == ext and any(
                                fil in filename.name for fil in file_filter_strings
                            ):
...
def GroupsNormalize(data, xaxis, yaxis, string):
    ...
    for density in data["Mesh_Density"].unique():
        for prefactor in data["Partition_Prefactor"].unique():
            group = data[
                (data["Mesh_Density"] == density)
                & (data["Partition_Prefactor"] == prefactor)
            ]
...
def MergeMetaData(directory, file_name, cohort_list, results, cat_columns):
    ...
    meta_data = pd.concat(
        [
            pd.read_excel(Path(directory) / file_name, sheet_name=sheet)
            for sheet in cohort_list
        ],
        ignore_index=True,
    )
